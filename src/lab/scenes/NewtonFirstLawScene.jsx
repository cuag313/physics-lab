...
export default function NewtonFirstLawScene() {
  ...
  const S = useRef({
    ...
    // 推理动画
    inferencePhase: 0,   // 0=实际, 1=摩擦减小, 2=理想
    inferenceTimer: 0,
  ...
  function drawInfoPanel(ctx, R) {
    ...
    // 理论距离
    if (s.friction > 0.001) {
      const vAtBottom = Math.sqrt(2 * s.g * s.rampLen * Math.sin(s.rampAngle * Math.PI / 180) * 0.67)
    ...
    // 状态
    ctx.fillStyle = s.ballPhase === 'stopped' ? '#F44336' : s.ballPhase === 'idle' ? '#484f58' : '#4CAF50'
    const statusText = {
      idle: '拖拽小球到斜面上释放',
      rolling: '在斜面上滚动...',
      sliding: '在水平面上滑动...',
  ...
  function drawDescription(ctx, R) {
    ...
    ctx.fillText('牛顿第一定律（惯性定律）', x, y); y += 20

    ctx.fillStyle = '#4FC3F7'
    ctx.font = 'bold 13px serif'
    ctx.fillText('物体不受外力时保持静止或匀速直线运动', x + 250, y - 20)
  }
