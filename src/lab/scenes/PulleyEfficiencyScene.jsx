...
/**
 * PulleyEfficiencyScene — 测量滑轮组机械效率（v10 按规范完全重写）
 * 所有绳段严格竖直，不穿滑轮圆心
...
export default function PulleyEfficiencyScene() {
  ...
  const S = useRef({ G: 10, G0: 2, n: 2, mu: 0.10, h: 0, dragging: false, dragStartY: 0, dragStartH: 0, hoverWeight: false, records: [], breathPhase: 0 })
  const [gVal, setGVal] = useState(10), [g0Val, setG0Val] = useState(2), [nVal, setNVal] = useState(2), [muVal, setMuVal] = useState(0.10), [hVal, setHVal] = useState(0)
  const [records, setRecords] = useState([]), [showConclusion, setShowConclusion] = useState(false)
  ...
  function getLayout() {
    const { n, h } = S.current
    const cx = FRAME_CX, fpx = cx, fpy = FRAME_TOP + 60
    const baseMpy = fpy + 150, hPx = (h / 1.5) * 130, mpy = baseMpy - hPx, mpx = cx
    const xL = mpx - R, xR = mpx + R
    const weightW = 64, weightH = 48, weightY = mpy + R + 16
  ...
  function drawFrame(ctx, L) {
    const { fpx, fpy } = L, bl = fpx - 55, br = fpx + 75
    ctx.strokeStyle = '#888'; ctx.lineCap = 'round'
    ctx.lineWidth = 5; ctx.beginPath(); ctx.moveTo(bl, FRAME_TOP); ctx.lineTo(bl, CANVAS_H - 10); ctx.stroke()
    ctx.lineWidth = 5; ctx.beginPath(); ctx.moveTo(bl - 5, FRAME_TOP); ctx.lineTo(br, FRAME_TOP); ctx.stroke()
    ctx.lineWidth = 6; ctx.beginPath(); ctx.moveTo(bl - 20, CANVAS_H - 10); ctx.lineTo(bl + 40, CANVAS_H - 10); ctx.stroke()
    ctx.strokeStyle = '#999'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(fpx, FRAME_TOP); ctx.lineTo(fpx, fpy - R); ctx.stroke()
  }
  ...
  function drawPulley(ctx, x, y, r, label) {
    ...
    ctx.strokeStyle = '#666'; ctx.lineWidth = 3; ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.stroke()
    ctx.fillStyle = '#444'; ctx.beginPath(); ctx.arc(x, y, 4, 0, Math.PI * 2); ctx.fill()
    ctx.fillStyle = '#fff'; ctx.font = 'bold 12px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'
    ctx.fillText(label, x, y)
  }

  function drawRope(ctx, L, calcs) {
    ...
    if (n === 2) {
      ...
      ctx.beginPath(); ctx.moveTo(xL, fpy); ctx.lineTo(xL, mpy); ctx.stroke()   // 左：定左→动左
      ctx.beginPath(); ctx.moveTo(xR, mpy); ctx.lineTo(xR, fpy); ctx.stroke()   // 右：动右→定右

      // 弧线（3px）
      ctx.lineWidth = 3
      ctx.beginPath(); ctx.arc(mpx, mpy, R, Math.PI, 2 * Math.PI, false); ctx.stroke() // 动滑轮下半圆
      ctx.beginPath(); ctx.arc(fpx, fpy, R, 0, Math.PI, false); ctx.stroke()            // 定滑轮上半圆
      ...
      ctx.beginPath(); ctx.moveTo(xL - 5, freeEndY - 6); ctx.lineTo(xL, freeEndY); ctx.lineTo(xL + 5, freeEndY - 6); ctx.fill()
      ...
      // 承重段（4px）
      ctx.strokeStyle = C; ctx.lineWidth = 4
      ctx.beginPath(); ctx.moveTo(mpx, mpy - R - 4); ctx.lineTo(xL, fpy); ctx.stroke() // 始端→定左
      ctx.beginPath(); ctx.moveTo(xR, fpy); ctx.lineTo(xR, mpy); ctx.stroke()           // 定右→动右
      ctx.beginPath(); ctx.moveTo(xL, mpy); ctx.lineTo(xL, freeEndY); ctx.stroke()      // 动左→自由端

      // 弧线（3px）
      ctx.lineWidth = 3
      ctx.beginPath(); ctx.arc(fpx, fpy, R, Math.PI, 2 * Math.PI, true); ctx.stroke()  // 定滑轮上半圆
      ctx.beginPath(); ctx.arc(mpx, mpy, R, 0, Math.PI, true); ctx.stroke()             // 动滑轮下半圆
  ...
  function drawWeight(ctx, L) {
    ...
    ctx.moveTo(x + rr, y); ctx.lineTo(x + weightW - rr, y); ctx.arcTo(x + weightW, y, x + weightW, y + rr, rr)
    ctx.lineTo(x + weightW, y + weightH - rr); ctx.arcTo(x + weightW, y + weightH, x + weightW - rr, y + weightH, rr)
    ctx.lineTo(x + rr, y + weightH); ctx.arcTo(x, y + weightH, x, y + weightH - rr, rr)
    ctx.lineTo(x, y + rr); ctx.arcTo(x, y, x + rr, y, rr); ctx.fill()
  ...
  function drawAnnotations(ctx, L, calcs) {
    ...
    // h标注
    if (h > 0.005) {
      const initCY = fpy + 150 + R + 16 + weightH / 2, curCY = weightY + weightH / 2, ax = 25
      ...
      ctx.beginPath(); ctx.moveTo(ax - 3, initCY - 6); ctx.lineTo(ax, initCY); ctx.lineTo(ax + 3, initCY - 6); ctx.fill()
      ctx.beginPath(); ctx.moveTo(ax - 3, curCY + 6); ctx.lineTo(ax, curCY); ctx.lineTo(ax + 3, curCY + 6); ctx.fill()
      ctx.font = 'bold 12px sans-serif'; ctx.textAlign = 'left'; ctx.fillText('h = ' + h.toFixed(2) + ' m', ax + 8, (initCY + curCY) / 2)
    }
    // 呼吸提示
  ...
  useEffect(() => {
    ...
    function render() {
      ...
      drawRope(ctx, L, calcs)
      drawPulley(ctx, L.fpx, L.fpy, R, '定')
      drawPulley(ctx, L.mpx, L.mpy, R, '动')
      drawWeight(ctx, L); drawAnnotations(ctx, L, calcs)
  ...
  const handleDown = useCallback((e) => {
    ...
    if (inW) { S.current.dragging = true; S.current.dragStartY = pos.y; S.current.dragStartH = S.current.h; canvas.style.cursor = 'grabbing' }
  }, [getPos])
  ...
  const handleMove = useCallback((e) => {
    ...
    if (!S.current.dragging) {
      const inW = pos.x >= L.mpx - L.weightW / 2 - 18 && pos.x <= L.mpx + L.weightW / 2 + 22 && pos.y >= L.weightY - 8 && pos.y <= L.weightY + L.weightH + 8
      S.current.hoverWeight = inW; if (canvas) canvas.style.cursor = inW ? 'grab' : 'default'; return
    }
    e.preventDefault()
    const dy = pos.y - S.current.dragStartY, dh = -dy / (130 / 1.5)
  ...
  const handleUp = useCallback(() => {
    ...
    const canvas = canvasRef.current; if (canvas) canvas.style.cursor = S.current.hoverWeight ? 'grab' : 'default'
  }, [])
  ...
  const clearRecords = useCallback(() => { if (window.confirm('确定清空？')) { setRecords([]); setShowConclusion(false) } }, [])
  ...
  const st = {
    ...
    nb: (a) => ({ padding: '2px 8px', borderRadius: 3, border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 600, background: a ? '#3498db' : 'transparent', color: a ? '#fff' : '#889' }),
  ...
  return (
    <div style={st.wrap}>
      <div style={st.top}>
        ...
        <div style={st.sg}>
          <div style={st.si}><span style={st.sl}>G物</span><input type="range" min={1} max={20} step={1} value={gVal} style={st.sr} onChange={e => updateG(Number(e.target.value))} /><span style={st.sv}>{gVal.toFixed(1)}N</span></div>
          <div style={st.si}><span style={st.sl}>G动</span><input type="range" min={0.5} max={5} step={0.5} value={g0Val} style={st.sr} onChange={e => updateG0(Number(e.target.value))} /><span style={st.sv}>{g0Val.toFixed(1)}N</span></div>
          <div style={st.si}><span style={st.sl}>μ</span><input type="range" min={0} max={0.3} step={0.01} value={muVal} style={st.sr} onChange={e => updateMu(Number(e.target.value))} /><span style={st.sv}>{muVal.toFixed(2)}</span></div>
          <div style={st.si}><span style={st.sl}>n</span><div style={st.nt}><button style={st.nb(nVal===2)} onClick={() => updateN(2)}>2段</button><button style={st.nb(nVal===3)} onClick={() => updateN(3)}>3段</button></div></div>
        </div>
        <div style={st.rb}>
      ...
      <div style={st.mn}>
        ...
        <div style={st.pn}>
          ...
          <div style={st.gd}>
            ...
            <div style={st.dv}/><span style={st.sc}>拉力分析</span>
            ...
            <div style={st.dv}/><span style={st.sc}>距离</span>
            ...
            <span style={{fontSize:10,color:'#999',gridColumn:'1/-1',textAlign:'center'}}>（s=n×h={nVal}×{calcs.h.toFixed(2)}={calcs.s.toFixed(2)}）</span>
            <div style={st.dv}/><span style={st.sc}>功与效率</span>
            ...
            <span style={{fontSize:10,color:'#999',textAlign:'right',paddingRight:16}}>{calcs.eta_ideal.toFixed(1)} %</span>
            {showConclusion && <div style={st.cn}><div style={{fontWeight:700,marginBottom:2}}>📋 结论</div><div>①物重越大效率越高 ②动滑轮越重效率越低 ③摩擦越大效率越低</div></div>}
            {records.length > 0 && <div style={{marginTop:4,gridColumn:'1/-1'}}><div style={{fontWeight:600,fontSize:11,marginBottom:2}}>📝 记录</div><table style={st.tb}><thead><tr>{['#','G物','G动','n','μ','h','s','F','W有','W总','η'].map(h=><th key={h} style={st.th}>{h}</th>)}</tr></thead><tbody>{records.map(r=><tr key={r.id}><td style={st.td}>{r.id}</td><td style={st.td}>{r.G}</td><td style={st.td}>{r.G0}</td><td style={st.td}>{r.n}</td><td style={st.td}>{r.mu.toFixed(2)}</td><td style={st.td}>{r.h.toFixed(2)}</td><td style={st.td}>{r.s.toFixed(2)}</td><td style={st.td}>{r.F.toFixed(2)}</td><td style={st.td}>{r.W_useful.toFixed(2)}</td><td style={st.td}>{r.W_total.toFixed(2)}</td><td style={st.td}>{r.eta.toFixed(1)}%</td></tr>)}</tbody></table></div>}
          </div>
        </div>
      </div>
      <div style={st.bt}>
