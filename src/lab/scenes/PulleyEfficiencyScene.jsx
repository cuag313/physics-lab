...
/**
 * PulleyEfficiencyScene — 测量滑轮组机械效率（v7 按规范重写）
 *
 * 绕线规则（奇动偶定）：
 * - n=2（偶数）：始端固定在定滑轮中心 → 动滑轮下半圆(左→下→右) → 定滑轮上半圆(右→上→左) → 自由端向下
 * - n=3（奇数）：始端固定在动滑轮中心 → 定滑轮上半圆(右→上→左) → 动滑轮下半圆(左→下→右) → 自由端向上
 *
 * Canvas弧线方向（arc-test.html实测）：
 * - arc(0, π, false) = 上半圆（右→上→左）
 * - arc(π, 2π, false) = 下半圆（左→下→右）
 *
 * 数值精度：h先四舍五入到两位小数，再参与所有计算
 */
...
const CANVAS_H = 540
const FRAME_TOP = 40
const FRAME_CX = 180   // 铁架台偏左，给右侧留空间
...
export default function PulleyEfficiencyScene() {
  ...
  const S = useRef({
    ...
    hoverWeight: false, hoverHandle: false,
  ...
  function calc() {
    ...
    const h = Math.round(S.current.h * 100) / 100   // 关键：先四舍五入
    const F_ideal = (G + G0) / n
    const F_friction = F_ideal * mu
    const F = F_ideal * (1 + mu)
    const s = Math.round(n * h * 100) / 100          // s = n × h
    const W_useful = Math.round(G * h * 100) / 100   // W有 = G物 × h
    const W_total = Math.round(F * s * 100) / 100    // W总 = F × s
    const W_extra = Math.round((W_total - W_useful) * 100) / 100
    const W_pulley = Math.round(G0 * h * 100) / 100  // W动 = G动 × h
  ...
  function getLayout() {
    ...
    // L形支架：左侧竖杆 + 顶部横杆
    const barLeft = cx - 50
    const barRight = cx + 70   // 横杆右端（不画右竖杆）
    const fpx = cx              // 定滑轮中心X
    const fpy = FRAME_TOP + 50  // 定滑轮中心Y
  ...
  function drawAnnotations(ctx, L, calcs) {
    ...
    if (n === 2) {
      ...
      ctx.fillText(`F = ${F.toFixed(2)} N`, handleX - 8, handleY - 6)
    } else {
      // 自由端向上
      drawLine(ctx, handleX, handleY + 18, handleX, handleY + 2)
      ctx.beginPath(); ctx.moveTo(handleX - 5, handleY + 7); ctx.lineTo(handleX, handleY); ctx.lineTo(handleX + 5, handleY + 7); ctx.fill()
    ...
    // 呼吸提示
    if (S.current.h === 0 && !S.current.dragging) {
  ...
  const handleDown = useCallback((e) => {
    ...
    const inH = dx * dx + dy * dy <= 484  // 22^2
  ...
  const handleMove = useCallback((e) => {
    ...
    if (!S.current.dragging) {
      ...
      const inW = pos.x >= mpx - weightW / 2 - 18 && pos.x <= mpx + weightW / 2 + 22 &&
                  pos.y >= weightY - 8 && pos.y <= weightY + weightH + 8
      const dx = pos.x - L.handleX, dy = pos.y - L.handleY
      const inH = dx * dx + dy * dy <= 484
      S.current.hoverWeight = inW
      S.current.hoverHandle = inH
      if (canvas) canvas.style.cursor = (inW || inH) ? 'grab' : 'default'
      return
    }
    ...
    const n = S.current.n
    // n=2：向下拖=h增大；n=3：向上拖=h增大（因为自由端向上）
    const dh = n === 2 ? -dy / 120 : dy / 120
  ...
  const handleUp = useCallback(() => {
    ...
    const canvas = canvasRef.current
    if (canvas) canvas.style.cursor = (S.current.hoverWeight || S.current.hoverHandle) ? 'grab' : 'default'
  }, [])
          ...
          <div style={st.grid}>
            ...
            {showConclusion && (
              <div style={st.conclusion}>
                <div style={{ fontWeight: 700, marginBottom: 3 }}>📋 实验结论</div>
                <div>① 物重越大，效率越高</div>
                <div>② 动滑轮越重，效率越低</div>
                <div>③ 摩擦越大，效率越低</div>
              </div>
            ...
            {records.length > 0 && (
              <div style={{ marginTop: 6, gridColumn: '1 / -1' }}>
                <div style={{ fontWeight: 600, fontSize: 12, marginBottom: 3 }}>📝 实验记录</div>
                <div style={{ overflowX: 'auto' }}>
                  <table style={st.table}>
                    <thead><tr>{['#','G物','G动','n','μ','h','s','F','W有','W总','η'].map(h => <th key={h} style={st.th}>{h}</th>)}</tr></thead>
                    <tbody>{records.map(r => (
                      <tr key={r.id}>
                        <td style={st.td}>{r.id}</td><td style={st.td}>{r.G}</td><td style={st.td}>{r.G0}</td>
                        <td style={st.td}>{r.n}</td><td style={st.td}>{r.mu.toFixed(2)}</td>
                        <td style={st.td}>{r.h.toFixed(2)}</td><td style={st.td}>{r.s.toFixed(2)}</td>
                        <td style={st.td}>{r.F.toFixed(2)}</td><td style={st.td}>{r.W_useful.toFixed(2)}</td>
                        <td style={st.td}>{r.W_total.toFixed(2)}</td><td style={st.td}>{r.eta.toFixed(1)}%</td>
                      </tr>
                    ))}</tbody>
                  </table>
                </div>
