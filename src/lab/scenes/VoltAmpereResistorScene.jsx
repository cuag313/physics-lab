 ...
 *
 * Tab1: 实验演示 — 电路符号原理图
 *   矩形回路：下=电源+开关，上=滑线变阻器→电流表→灯泡，灯泡并联电压表
 *   滑线变阻器改变总电阻 → 改变电流 → 灯泡亮度变化（功率决定）
 *
 * Tab2: 自己动手 — 实物器材图片
 *   初始空白，从器材栏拖入；接线柱画在器材上；导线有铆点可拉折角
 */
...
export default function VoltAmpereResistorScene() {
  ...
  const diy = useRef({
    ...
    nextId: 1,
    wireErrors: [], circuitStatus: { ok: false, reason: '' },
    undoStack: [], redoStack: [],
    showGuide: true,
  ...
  useEffect(() => {
    const list = {
      ...
      bulb: './assets/equipment/bulb_on.png',
  ...
  useEffect(() => {
    ...
    const R = {
      cv, ctx: cv.getContext('2d'), W: 0, H: 0,
      resize() {
  ...
  function updatePhysics(s) {
    if (!s.switchClosed) {
      ...
      s.overRangeA = false; s.overRangeV = false; return
    }
    const Rtot = s.R_true + s.sliderR
    const I = Rtot > 0 ? s.U_source / Rtot : 0
    const UR = I * s.R_true
  ...
  function renderDemo(ctx, W, H, s) {
    ...
    const Rtot = s.R_true + s.sliderR
    const I = on && Rtot > 0 ? s.U_source / Rtot : 0
    const UR = I * s.R_true
    const brightness = on ? Math.min(1, (I * I * s.R_true) / 3.6) : 0

    // 电路区域（画布左侧60%）
    const areaL = 30, areaR = W * 0.56, areaT = 70, areaB = H - 60
    const midX = (areaL + areaR) / 2

    // 矩形回路坐标
    const btmY = areaB - 40        // 下边Y（电源+开关）
    const topY = areaT + 30        // 上边Y（变阻器+电流表+灯泡）
    const leftX = areaL + 60       // 左边X（电源→灯泡）
    const rightX = areaR - 60      // 右边X（开关→变阻器）

    // 元件位置
    const batX = leftX + 50        // 电源（下边偏左）
    const swX = rightX - 50        // 开关（下边偏右）
    const rheoX = leftX + 50       // 滑线变阻器（上边偏左）
    const ammX = midX              // 电流表（上边中间）
    const bulbX = rightX - 50      // 灯泡（上边偏右）

    const wc = on ? '#1565C0' : '#999'

    // ─── 导线（矩形回路）───
    // 下边：电源→开关
    drawLn(ctx, batX + 14, btmY, swX - 18, btmY, wc)
    // 右边：开关→上边变阻器
    drawLn(ctx, swX, btmY, swX, topY, wc)
    // 上边左段：变阻器→电流表
    drawLn(ctx, rheoX + 24, topY, ammX - 18, topY, wc)
    // 上边右段：电流表→灯泡
    drawLn(ctx, ammX + 18, topY, bulbX - 14, topY, wc)
    // 左边：电源→变阻器
    drawLn(ctx, batX, btmY, batX, topY, wc)
    // 左边上段连接变阻器
    drawLn(ctx, batX, topY, rheoX - 24, topY, wc)
    ...
    if (on) {
      drawFlow(ctx, [
        { x: batX + 14, y: btmY }, { x: swX, y: btmY },
        { x: swX, y: topY }, { x: rheoX, y: topY },
        { x: ammX, y: topY }, { x: bulbX, y: topY },
        { x: batX, y: topY }, { x: batX, y: btmY },
      ], s.time)
      // 导线高亮
      ctx.strokeStyle = 'rgba(21,101,225,0.12)'; ctx.lineWidth = 6
      ctx.beginPath(); ctx.moveTo(batX, btmY); ctx.lineTo(swX, btmY); ctx.stroke()
      ctx.beginPath(); ctx.moveTo(swX, btmY); ctx.lineTo(swX, topY); ctx.stroke()
      ctx.beginPath(); ctx.moveTo(rheoX, topY); ctx.lineTo(bulbX, topY); ctx.stroke()
      ctx.beginPath(); ctx.moveTo(batX, topY); ctx.lineTo(batX, btmY); ctx.stroke()
    }
    ...
    // 滑线变阻器
    drawRheoSym(ctx, rheoX, topY, s.sliderR, 50)
    // Ⓐ 电流表
    ...
    ctx.fillText(on ? 'ON' : 'OFF', swX, btmY + 22)
    ctx.fillText('滑线变阻器', rheoX, topY - 24)
    ctx.fillText('灯泡', bulbX, topY - 24)
    ctx.fillStyle = '#888'; ctx.font = '9px sans-serif'
    ctx.fillText('Ⓐ串联测电流', ammX, topY - 24)
    ctx.fillText('Ⓥ并联测电压', bulbX, volY + 24)
    ...
    ctx.fillText('原理：R = U / I', px + 14, ky); ky += 16
    ctx.fillText('灯泡电阻不变，改变滑线变阻器电阻', px + 14, ky); ky += 16
    ctx.fillText('→ 总电阻变化 → 电流变化 → 灯泡亮度变化', px + 14, ky); ky += 24
    ...
    if (on) {
      ...
      ctx.fillText('电源 U = ' + s.U_source.toFixed(1) + ' V', px + 14, ky); ky += 16
      ctx.fillText('滑动变阻器 = ' + s.sliderR + ' Ω', px + 14, ky); ky += 16
      ctx.fillText('灯泡电阻 = ' + s.R_true + ' Ω', px + 14, ky); ky += 16
      ctx.fillText('总电阻 = ' + Rtot.toFixed(1) + ' Ω', px + 14, ky); ky += 16
      ...
      ctx.fillText('灯泡电阻 R = ' + (I > 0.001 ? (UR / I).toFixed(1) : '—') + ' Ω', px + 14, ky); ky += 20

      // 亮度说明
      const P = I * I * s.R_true
      ctx.fillStyle = '#FF9800'; ctx.font = '11px sans-serif'
      ctx.fillText('灯泡功率 P = I²R = ' + P.toFixed(2) + ' W', px + 14, ky); ky += 14
      ctx.fillStyle = '#888'; ctx.font = '10px sans-serif'
      ctx.fillText('功率越大 → 灯泡越亮', px + 14, ky); ky += 20
    }
    ...
    if (s.data.length > 0) {
      ...
      if (s.data.length >= 3) {
        ky += 6; const avgR = s.data.reduce((a, d) => a + d.R, 0) / s.data.length
        ctx.fillStyle = '#E53935'; ctx.font = 'bold 12px sans-serif'
        ctx.fillText('R̄ = ' + avgR.toFixed(1) + ' Ω', px + 14, ky); ky += 16
      ...
      ctx.fillText('2. 调节滑动变阻器改变电流', px + 14, ky); ky += 14
      ctx.fillText('3. 观察灯泡亮度变化', px + 14, ky); ky += 14
      ctx.fillText('4. 点"记录"保存U、I数据', px + 14, ky); ky += 14
      ctx.fillText('5. 至少记录3组，求电阻平均值', px + 14, ky)
    }
    ctx.textBaseline = 'alphabetic'
    ...
    if (recOk) canvasRef.current._clickAreas.push({ x: 20, y: nY, w: 90, h: 30, onClick: () => {
      const Rtot2 = s.R_true + s.sliderR, Iv = Rtot2 > 0 ? s.U_source / Rtot2 : 0, URv = Iv * s.R_true
      s.data.push({ sliderR: s.sliderR, U: URv, I: Iv, R: Iv > 0 ? URv / Iv : 0 }); forceUpdate(n => n + 1)
    }})
  ...
  function renderDIY(ctx, W, H, s) {
    ...
    if (s.showGuide && s.components.length === 0) {
      ...
      ctx.fillText('← 从右侧器材栏拖拽器材到画布中', cvX + cvW / 2, cvY + cvH / 2 - 10)
      ctx.fillStyle = '#888'; ctx.font = '11px sans-serif'
      ctx.fillText('点击接线柱连线 · 导线铆点可拖拽折角 · 双击开关切换', cvX + cvW / 2, cvY + cvH / 2 + 15)
      ctx.textBaseline = 'alphabetic'
    }
    ...
    const items = [
      { type: 'battery', name: '干电池组' }, { type: 'switch', name: '单向开关' },
      { type: 'rheostat', name: '滑线变阻器' }, { type: 'ammeter', name: '电流表' },
      { type: 'bulb', name: '灯泡' }, { type: 'voltmeter', name: '电压表' },
    ]
    let iy = cvY + 30
    ...
    canvasRef.current._clickAreas.push({ x: 20, y: nY, w: 110, h: 30, onClick: () => {
      s.switchClosed = !s.switchClosed
      const sw = s.components.find(c => c.type === 'switch'); if (sw) sw.closed = s.switchClosed
      forceUpdate(n => n + 1)
    }})
    // 记录
    ...
    if (recOk) canvasRef.current._clickAreas.push({ x: 140, y: nY, w: 90, h: 30, onClick: () => {
      const Rtot2 = s.R_true + s.sliderR, Iv = Rtot2 > 0 ? s.U_source / Rtot2 : 0, URv = Iv * s.R_true
      s.data.push({ sliderR: s.sliderR, U: URv, I: Iv, R: Iv > 0 ? URv / Iv : 0 }); forceUpdate(n => n + 1)
    }})
    // 重置
    ...
    canvasRef.current._clickAreas.push({ x: 240, y: nY, w: 80, h: 30, onClick: () => {
      s.U_source = DEFAULTS.U_source; s.R_true = DEFAULTS.R_true; s.sliderR = DEFAULTS.sliderR
      s.data = []; s.switchClosed = false; s.wires = []; s.components = []; s.undoStack = []; s.redoStack = []
      forceUpdate(n => n + 1)
    }})
    ctx.textBaseline = 'alphabetic'
    ...
    if (s.data.length > 0) {
      ...
      ctx.fillText('已记录 ' + s.data.length + ' 组', 340, nY + 15)
      if (s.data.length >= 3) { const avgR = s.data.reduce((a, d) => a + d.R, 0) / s.data.length; ctx.fillStyle = '#E53935'; ctx.font = 'bold 11px sans-serif'; ctx.fillText('R̄=' + avgR.toFixed(1) + 'Ω', 430, nY + 15) }
      ctx.textBaseline = 'alphabetic'
    }
  }
  ...
  function drawComp(ctx, comp, s, I, UR) {
    ...
    if (type === 'battery') {
      const img = imgCache.current.battery; if (img) drawImgFit(ctx, img, 0, 0, 90, 60)
      else { ctx.fillStyle = '#81C784'; ctx.strokeStyle = '#388E3C'; ctx.lineWidth = 2; ctx.beginPath(); ctx.roundRect(-40, -25, 80, 50, 8); ctx.fill(); ctx.stroke() }
      ctx.fillStyle = '#E53935'; ctx.font = 'bold 10px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'bottom'; ctx.fillText('+', -44, -26); ctx.fillStyle = '#333'; ctx.fillText('−', 44, -18)
    } else if (type === 'switch') {
      const img = imgCache.current.switch; if (img) { drawImgFit(ctx, img, 0, 0, 80, 44); ctx.fillStyle = comp.closed !== false ? 'rgba(76,175,80,0.12)' : 'rgba(244,67,54,0.08)'; ctx.beginPath(); ctx.arc(0, 0, 25, 0, Math.PI * 2); ctx.fill() }
      ...
      // ON/OFF 标记
      ctx.fillStyle = comp.closed !== false ? '#4CAF50' : '#F44336'; ctx.font = 'bold 10px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'top'; ctx.fillText(comp.closed !== false ? 'ON' : 'OFF', 0, 24)
      ...
      else { ctx.fillStyle = '#FFEBEE'; ctx.strokeStyle = '#E53935'; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(0, 0, 30, 0, Math.PI * 2); ctx.fill(); ctx.stroke(); ctx.fillStyle = '#E53935'; ctx.font = 'bold 16px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('A', 0, 0) }
      ...
      else { ctx.fillStyle = '#E8F5E9'; ctx.strokeStyle = '#4CAF50'; ctx.lineWidth = 2; ctx.beginPath(); ctx.arc(0, 0, 30, 0, Math.PI * 2); ctx.fill(); ctx.stroke(); ctx.fillStyle = '#4CAF50'; ctx.font = 'bold 16px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('V', 0, 0) }
      ...
      ctx.fillStyle = '#555'; ctx.font = '9px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'top'; ctx.fillText('灯泡', 0, 38)
    }

    ctx.globalAlpha = 1; ctx.restore()

    // 接线柱（画在器材边缘上，小圆圈）
    const terms = getTerms(comp)
    for (let i = 0; i < terms.length; i++) {
      const t = terms[i]
      const hov = s.hoverTerm && s.hoverTerm.compId === comp.id && s.hoverTerm.termIdx === i
      const snap = s.connecting && hov
      ctx.fillStyle = snap ? '#4CAF50' : hov ? '#FF9800' : '#fff'
      ctx.strokeStyle = snap ? '#2E7D32' : hov ? '#E65100' : '#666'
      ctx.lineWidth = snap ? 3 : 2
      ctx.beginPath(); ctx.arc(t.x, t.y, snap ? 8 : 5, 0, Math.PI * 2); ctx.fill(); ctx.stroke()
      if (snap) { ctx.strokeStyle = 'rgba(76,175,80,0.3)'; ctx.lineWidth = 1; ctx.beginPath(); ctx.arc(t.x, t.y, 14, 0, Math.PI * 2); ctx.stroke() }
    }
  }
  ...
  function drawDIYWire(ctx, wire, s) {
    ...
    // 错误标记
    if (err) { const mx = (f.x + t.x) / 2, my = (f.y + t.y) / 2; ctx.fillStyle = '#F44336'; ctx.font = 'bold 10px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('✗', mx, my - 10) }
  }
  ...
  function drawBat(ctx, x, y) {
    ctx.strokeStyle = '#333'; ctx.lineWidth = 2; ctx.beginPath(); ctx.moveTo(x - 12, y - 18); ctx.lineTo(x - 12, y + 18); ctx.stroke()
    ctx.lineWidth = 5; ctx.beginPath(); ctx.moveTo(x + 12, y - 9); ctx.lineTo(x + 12, y + 9); ctx.stroke()
    ctx.fillStyle = '#E53935'; ctx.font = 'bold 11px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'bottom'; ctx.fillText('+', x - 12, y - 20)
    ctx.fillStyle = '#333'; ctx.font = 'bold 14px sans-serif'; ctx.fillText('−', x + 12, y - 11)
    ctx.textBaseline = 'alphabetic'
  }

  function drawSw(ctx, x, y, on, onClick) {
    ctx.fillStyle = '#666'; ctx.beginPath(); ctx.arc(x - 18, y, 4, 0, Math.PI * 2); ctx.fill(); ctx.beginPath(); ctx.arc(x + 18, y, 4, 0, Math.PI * 2); ctx.fill()
    if (on) { ctx.strokeStyle = '#4CAF50'; ctx.lineWidth = 3; ctx.lineCap = 'round'; ctx.beginPath(); ctx.moveTo(x - 18, y); ctx.lineTo(x + 18, y); ctx.stroke(); ctx.lineCap = 'butt' }
    else { ctx.strokeStyle = '#F44336'; ctx.lineWidth = 3; ctx.lineCap = 'round'; ctx.beginPath(); ctx.moveTo(x - 18, y); ctx.lineTo(x + 10, y - 20); ctx.stroke(); ctx.lineCap = 'butt' }
    ctx.fillStyle = on ? '#4CAF50' : '#F44336'; ctx.font = 'bold 10px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'top'; ctx.fillText(on ? 'ON' : 'OFF', x, y + 8)
    ctx.textBaseline = 'alphabetic'
    canvasRef.current._clickAreas.push({ x: x - 25, y: y - 25, w: 50, h: 50, onClick })
  }

  function drawRheoSym(ctx, x, y, R, maxR) {
    // 电阻丝矩形
    ...
    // 滑片位置
    const ratio = R / maxR, sliderX = x - 24 + ratio * 48
    ...
    ctx.fillStyle = '#333'; ctx.font = '9px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'top'; ctx.fillText(R + 'Ω', x, y + 12)
    ctx.textBaseline = 'alphabetic'
  }

  function drawMeterSym(ctx, x, y, type, angle, over, reading) {
    const r = 22, color = type === 'A' ? '#E53935' : '#4CAF50'
    if (over) { const f = Math.sin(Date.now() / 150) > 0; ctx.fillStyle = f ? 'rgba(244,67,54,0.2)' : 'rgba(244,67,54,0.06)'; ctx.beginPath(); ctx.arc(x, y, r + 6, 0, Math.PI * 2); ctx.fill() }
    ctx.fillStyle = '#fff'; ctx.strokeStyle = over ? '#F44336' : color; ctx.lineWidth = 2.5; ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.fill(); ctx.stroke()
    ctx.fillStyle = color; ctx.font = 'bold 16px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(type, x, y - 4)
    const a = -Math.PI * 0.6 + Math.min(angle, 1.5) * Math.PI * 1.2
    ctx.strokeStyle = '#333'; ctx.lineWidth = 1.5; ctx.lineCap = 'round'; ctx.beginPath(); ctx.moveTo(x, y); ctx.lineTo(x + Math.cos(a) * (r - 6), y + Math.sin(a) * (r - 6)); ctx.stroke(); ctx.lineCap = 'butt'
    if (reading) { ctx.fillStyle = over ? '#F44336' : '#333'; ctx.font = 'bold 10px monospace'; ctx.textAlign = 'center'; ctx.textBaseline = 'top'; ctx.fillText(reading, x, y + 10) }
    if (over) { ctx.fillStyle = '#F44336'; ctx.font = 'bold 9px sans-serif'; ctx.fillText('超量程!', x, y + r + 12) }
    ctx.textBaseline = 'alphabetic'
  }

  function drawBulbSym(ctx, x, y, brightness) {
    const r = 14
    ctx.fillStyle = brightness > 0.3 ? '#FFEB3B' : '#f5f5f5'; ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.fill()
    ctx.strokeStyle = brightness > 0.3 ? '#F9A825' : '#999'; ctx.lineWidth = 1.5; ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.stroke()
    // × 灯丝
    ctx.strokeStyle = brightness > 0.3 ? '#E65100' : '#999'; ctx.lineWidth = 1.5
    const s2 = r * 0.55; ctx.beginPath(); ctx.moveTo(x - s2, y - s2); ctx.lineTo(x + s2, y + s2); ctx.stroke(); ctx.beginPath(); ctx.moveTo(x + s2, y - s2); ctx.lineTo(x - s2, y + s2); ctx.stroke()
    // 发光
    if (brightness > 0.3) { const glow = ctx.createRadialGradient(x, y, r, x, y, r * 3); glow.addColorStop(0, 'rgba(255,235,59,' + (brightness * 0.3) + ')'); glow.addColorStop(1, 'rgba(255,235,59,0)'); ctx.fillStyle = glow; ctx.beginPath(); ctx.arc(x, y, r * 3, 0, Math.PI * 2); ctx.fill() }
    // 底座
  ...
  function drawFlow(ctx, pts, t) {
    let len = 0; const segs = []; for (let i = 0; i < pts.length - 1; i++) { const dx = pts[i+1].x - pts[i].x, dy = pts[i+1].y - pts[i].y, l = Math.sqrt(dx*dx+dy*dy); segs.push({ ...pts[i], ex: pts[i+1].x, ey: pts[i+1].y, l }); len += l }
    ctx.fillStyle = '#FFEB3B'; const n = Math.max(5, Math.floor(len / 45))
    for (let d = 0; d < n; d++) { let pos = ((t * 50 + d * (len / n)) % len); for (const seg of segs) { if (pos <= seg.l) { const r = pos / seg.l; ctx.beginPath(); ctx.arc(seg.x + (seg.ex - seg.x) * r, seg.y + (seg.ey - seg.y) * r, 3, 0, Math.PI * 2); ctx.fill(); break }; pos -= seg.l } }
  }
  ...
  function getTerms(comp) {
    ...
    return (off[comp.type] || [{ x: -30, y: 0 }, { x: 30, y: 0 }]).map(o => ({ x: comp.x + o.x, y: comp.y + o.y }))
  }
  ...
  const handleContextMenu = useCallback((e) => {
    if (tabRef.current !== 2) return; e.preventDefault(); const el = canvasRef.current; if (!el) return; const { x, y } = getPos(e), s = diy.current
    ...
    if (comp) { saveUndo(s); s.components = s.components.filter(c => c.id !== comp.id); s.wires = s.wires.filter(w => w.from.compId !== comp.id && w.to.compId !== comp.id) }
      ...
      for (const wire of s.wires) {
        const fc = s.components.find(c => c.id === wire.from.compId), tc = s.components.find(c => c.id === wire.to.compId)
        if (!fc || !tc) continue; const f = termPos(fc, wire.from.termIdx), t = termPos(tc, wire.to.termIdx)
        const dx = t.x - f.x, dy = t.y - f.y, len2 = dx * dx + dy * dy; if (len2 === 0) continue
        const tp = Math.max(0, Math.min(1, ((x - f.x) * dx + (y - f.y) * dy) / len2)), px = f.x + tp * dx, py = f.y + tp * dy
        if ((x - px) ** 2 + (y - py) ** 2 < 100) { saveUndo(s); s.wires = s.wires.filter(w => w.id !== wire.id); break }
      }
    }
    forceUpdate(n => n + 1)
  }, [])
  ...
  return (
    <div style={st.container}>
      ...
      <div style={st.desc}><b>伏安法测定值电阻</b><span style={{ marginLeft: 12, color: '#555', fontSize: 13 }}>{tab === 1 ? '电路符号图 · 调节滑动变阻器改变电流 · 灯泡亮度随功率变化 · 记录数据求电阻' : '从器材栏拖入实物器材 · 接线柱吸附连线 · 铆点可拉折角 · 双击开关切换'}</span></div>
    </div>
  )
}
